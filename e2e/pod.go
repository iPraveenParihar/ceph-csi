...
func verifyUserIdMappingMetadata(
	...
	// verify userId mapping metadata is removed after pod deletion
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		if err != nil && strings.Contains(err.Error(), "command terminated with exit code 2") {
			framework.Logf("userId mapping metadata %s successfully removed", metadataKey)
			break
		}

		framework.Logf("userId mapping metadata %s still exists with value %s, retrying (%d/%d)",
			metadataKey, metadataValue, attempt, maxRetries)

		if attempt == maxRetries {
			return fmt.Errorf("userId mapping metadata %s still exists with value %s (after %d attempts)",
