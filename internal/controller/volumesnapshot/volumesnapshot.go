...
func (r *ReconcileVolumeSnapshot) reconcileVS(ctx context.Context, obj runtime.Object) error {
	...
	vscNamespace := vsc.Spec.VolumeSnapshotRef.Namespace
	...
	if ok := r.Locks.TryAcquire(*vsc.Spec.Source.SnapshotHandle); !ok {
		return fmt.Errorf("")
	}
	defer r.Locks.Release(*vsc.Spec.Source.SnapshotHandle)
	...
	if rbdVolID != snapshotHandle {
		log.DebugLog(ctx, "volumeHandler changed from %s to %s", snapshotHandle, rbdVolID)
	}
